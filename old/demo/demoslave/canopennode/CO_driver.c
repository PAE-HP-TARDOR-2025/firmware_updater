...
#ifndef CONFIG_DEMO_SLAVE_TWAI_TX_GPIO
#define CONFIG_DEMO_SLAVE_TWAI_TX_GPIO 5
#endif
#ifndef CONFIG_DEMO_SLAVE_TWAI_RX_GPIO
#define CONFIG_DEMO_SLAVE_TWAI_RX_GPIO 4
#endif

// Map sdkconfig values into the TWAI driver enums.
#define CAN_TX_GPIO     ((gpio_num_t)CONFIG_DEMO_SLAVE_TWAI_TX_GPIO)
#define CAN_RX_GPIO     ((gpio_num_t)CONFIG_DEMO_SLAVE_TWAI_RX_GPIO)
...
CO_CANsetNormalMode(CO_CANmodule_t* CANmodule) {
    ...
    if (driver_is_installed) {
        if (twai_start() == ESP_OK) {
            ...
            ESP_LOGI(TAG, "TWAI driver started (TX=%d RX=%d)", (int)CAN_TX_GPIO, (int)CAN_RX_GPIO);
        }
    }
}
...
CO_CANmodule_init(CO_CANmodule_t* CANmodule, void* CANptr, CO_CANrx_t rxArray[], uint16_t rxSize, CO_CANtx_t txArray[],
    ...
    if (!driver_is_installed) {
        ESP_LOGI(TAG, "Installing TWAI driver on TX=%d RX=%d...", (int)CAN_TX_GPIO, (int)CAN_RX_GPIO);
        ...
        switch (CANbitRate) {
            ...
            case 1000:
                ...
                break;
            default:
                ESP_LOGW(TAG, "Unsupported bit rate %u kbps, falling back to 500 kbps", CANbitRate);
                t_config = (twai_timing_config_t)TWAI_TIMING_CONFIG_500KBITS();
                break;
        }
...
CO_CANsend(CO_CANmodule_t* CANmodule, CO_CANtx_t* buffer) {
    ...
    if (CANmodule->CANtxCount == 0) {
        ...
        // DLC is encoded in bits 11-14 of buffer->ident by CO_CANtxBufferInit
        msg.data_length_code = (buffer->ident >> 11) & 0x0FU;
        msg.extd = 0; 
        msg.rtr = (buffer->ident & 0x8000U) ? 1 : 0;
        msg.ss = 0; 
        msg.self = 0; 
        msg.dlc_non_comp = 0;
        ...
        ESP_LOGI(TAG, "TX: id=0x%03lX dlc=%d data=[%02X %02X %02X %02X %02X %02X %02X %02X]",
                 (unsigned long)msg.identifier, msg.data_length_code,
                 msg.data[0], msg.data[1], msg.data[2], msg.data[3],
                 msg.data[4], msg.data[5], msg.data[6], msg.data[7]);

        // Wait up to 100ms for TX to complete (important for SDO responses!)
        TickType_t waitTicks = pdMS_TO_TICKS(100);
        if (waitTicks == 0) waitTicks = 1;
        
        esp_err_t txResult = twai_transmit(&msg, waitTicks);
        if (txResult == ESP_OK) {
            CANmodule->bufferInhibitFlag = buffer->syncFlag;
            CANmodule->firstCANtxMessage = false;
            ESP_LOGD(TAG, "TX success");
        } else {
            ESP_LOGW(TAG, "TWAI TX failed: %s (id=0x%03lX)", esp_err_to_name(txResult), (unsigned long)msg.identifier);
            // Mark buffer for retry
...
void CO_CANinterrupt(CO_CANmodule_t* CANmodule) {
    ...
    do {
        ...
        if (!matched) {
            ESP_LOGW(TAG, "RX: no matching buffer for id=0x%03lX", (unsigned long)msg.identifier);
        }
